...
__device__ void softmax_warp_inplace(float (&vals)[experts_per_thread], const int limit, const int lane) {
    float max_val = ggml_cuda_neginf_f();
...
#pragma unroll
    for (int i = 0; i < experts_per_thread; i++) {
        const int  idx    = lane + i * WARP_SIZE;
        const bool active = !use_limit || (idx < limit);
        vals[i]           = active ? 1.f / (1.f + expf(-vals[i])) : ggml_cuda_neginf_f();
    }
}
...
__launch_bounds__(4 * WARP_SIZE, 1) __global__ void topk_moe_cuda(const float *         logits,
    ...
    // Initialize all slots to ggml_cuda_neginf_f()
#pragma unroll
    for (int i = 0; i < experts_per_thread; i++) {
        wt[i] = ggml_cuda_neginf_f();
    }
...
#pragma unroll
    for (int i = 0; i < n_experts; i += WARP_SIZE) {
        ...
        wt[i / WARP_SIZE] = (n_experts % WARP_SIZE == 0 || expert < n_experts) ? logits[expert] : ggml_cuda_neginf_f();
    }
    ...
    // ggml_cuda_neginf_f() sentinel used after each selection round.
...
#pragma unroll
        for (int i = 0; i < experts_per_thread; i++) {
            selection_wt[i] = ggml_cuda_neginf_f();
        }
#pragma unroll
        for (int i = 0; i < n_experts; i += WARP_SIZE) {
            const int expert = i + threadIdx.x;
            selection_wt[i / WARP_SIZE] =
                (n_experts % WARP_SIZE == 0 || expert < n_experts) ? wt[i / WARP_SIZE] + bias[expert] : ggml_cuda_neginf_f();
        }
    }
...
#pragma unroll
            ...
            if ((max_expert & (WARP_SIZE - 1)) == threadIdx.x) {
                selection_wt[max_expert / WARP_SIZE] = ggml_cuda_neginf_f();
            }
        } else {
#pragma unroll
            for (int i = 1; i < experts_per_thread; i++) {
...
#pragma unroll
            ...
            if ((max_expert & (WARP_SIZE - 1)) == threadIdx.x) {
                wt[max_expert / WARP_SIZE] = ggml_cuda_neginf_f();
            }
        }
...
void ggml_cuda_op_topk_moe(ggml_backend_cuda_context &     ctx,
    ...
    float clamp_val = -std::numeric_limits<float>::infinity();
    if (clamp) {
