...
static __global__ void soft_max_f32(
    ...
    float max_val = sinks ? sinks[i02] : ggml_cuda_neginf_f();
...
static __device__ void soft_max_f32_parallelize_cols_single_row(const float * __restrict__ x,
    ...
    float     local_vals[n_elem_per_thread] = { ggml_cuda_neginf_f(), ggml_cuda_neginf_f(), ggml_cuda_neginf_f(), ggml_cuda_neginf_f() };
    float     local_max                     = ggml_cuda_neginf_f();
...
#pragma unroll
        for (int i = 0; i < n_elem_per_thread; i++) {
            ...
            local_vals[i] = idx < p.ncols ? x[idx] : ggml_cuda_neginf_f();
        }
#pragma unroll
        for (int i = 0; i < n_elem_per_thread; i++) {
...
#pragma unroll
        for (int i = 0; i < n_elem_per_thread; i++) {
            const int idx = col + i * step_size;
            local_vals[i] = idx < p.ncols ? dst[idx] : ggml_cuda_neginf_f();
        }
#pragma unroll
        for (int i = 0; i < n_elem_per_thread; i++) {
            const int idx = col + i * step_size;
