        ...
        default:
            {
                ...
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    if (arch == LLM_ARCH_GEMMA3N || arch == LLM_ARCH_GEMMA4 || arch == LLM_ARCH_GEMMA4_ASSISTANT) {
                        reuse = [&](int32_t il) {
...
int32_t llama_model_dspark_get_target_layers(
        const llama_model * model,
              int32_t     * out,
              int32_t       n_out) {
    if (model == nullptr || out == nullptr || n_out <= 0) {
        return 0;
    }

    const auto & hp = model->hparams;
    if (hp.dspark_block_size == 0 || hp.n_dspark_target_layers == 0) {
        return 0;
    }

    if ((int32_t) hp.n_dspark_target_layers > n_out) {
        return 0;
    }

    for (uint32_t i = 0; i < hp.n_dspark_target_layers; ++i) {
        out[i] = (int32_t) hp.dspark_target_layers[i];
    }

    return (int32_t) hp.n_dspark_target_layers;
}
