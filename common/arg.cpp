...
static ggml_type kvarn_fallback_cache_type(int32_t bits) {
    switch (bits) {
        // Q2_0/Q3_0 lack CUDA flash-attn KV kernels; use Q4_0 for iSWA fallback layers.
...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--allow-extended-ctx"},
        string_format(
            "allow context size beyond the model training length (requires YaRN/rope scaling; default: %s)",
            params.allow_extended_ctx ? "enabled" : "disabled"),
        [](common_params & params) {
            params.allow_extended_ctx = true;
        }
