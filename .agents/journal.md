...
- **Findings & Decisions**:
  ...
  - Resolved subsequent failures in GCC and Clang builds: added missing declaration prototype for `llama_get_ctx_other` in `llama-ext.h` (fixing `-Wmissing-prototypes`/`-Wmissing-declarations`) and refactored a string copy in `llama-triattention.cpp` using `snprintf` (fixing `-Wstringop-truncation`).
