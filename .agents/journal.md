...
- **Changes Completed**:
  ...
  - Modified [.github/workflows/release.yml](file:///J:/LLM/godzilla-llama.cpp/.github/workflows/release.yml) to broaden the OpenMP redist DLL directory lookup so it searches recursively from the Enterprise directory, preventing packing failure on the new Windows 2025 runner.
...
- **Findings & Decisions**:
  ...
  - Fixed Windows OpenMP package bundling failure: broadened search path and wildcard filter in `release.yml` for OpenMP DLL under the Visual Studio Enterprise directory to handle the new environment layout of `windows-2025` runners.
