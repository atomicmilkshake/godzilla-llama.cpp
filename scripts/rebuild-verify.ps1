...
try {
    cmake -S . -B build -G Ninja `
        ...
        Out-File J:\LLM\cmake-config.log
    $cfgExit = $LASTEXITCODE
    cmake --build build --config Release -j 8 --target ggml-cuda 2>&1 |
        Out-File J:\LLM\build-ggml-full.log
    $buildExit = $LASTEXITCODE
    Write-Host "configure=$cfgExit build=$buildExit"
    if ($buildExit -eq 0) {
        $warns = Select-String -Path J:\LLM\build-ggml-full.log -Pattern 'warning|C4244|C4005|#221|#177|#128' -CaseSensitive:$false
        if ($warns) {
